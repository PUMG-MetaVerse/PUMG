...
public class ExitDoor_2F_2_AnimationController : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            && ActionController.hitInfo.transform.name == gameObject.name)
        {
            if (Input.GetKeyDown(KeyCode.F))
            {
                ...
                photonView.RPC("RPC_ChangeExitDoor_2F_2_OpenState", RpcTarget.All, viewId);
            }
        }
    }
    ...
    [PunRPC]
    void RPC_ChangeExitDoor_2F_2_OpenState(int viewId)
    {
        PhotonView itemPhotonView = PhotonView.Find(viewId);

        if (itemPhotonView != null)
        {
            itemPhotonView.gameObject.GetComponent<ExitDoor_2F_2_AnimationController>().isExitDoorOpen =
                !itemPhotonView.gameObject.GetComponent<ExitDoor_2F_2_AnimationController>().isExitDoorOpen;
        }
    }
}
