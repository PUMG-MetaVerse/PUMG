...
public class Dresser_2F_1_AnimationController : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            && ActionController.hitInfo.transform.name == gameObject.name)
        {
            if (Input.GetKeyDown(KeyCode.F))
            {
                ...
                photonView.RPC("RPC_ChangeDresser_2F_1_OpenState", RpcTarget.All, viewId);
            }
        }
    }
    ...
    void RPC_ChangeDresser_2F_1_OpenState(int viewId)
    {
        PhotonView itemPhotonView = PhotonView.Find(viewId);

        if (itemPhotonView != null)
        {
            itemPhotonView.gameObject.GetComponent<Dresser_2F_1_AnimationController>().isDresserOpen =
                !itemPhotonView.gameObject.GetComponent<Dresser_2F_1_AnimationController>().isDresserOpen;
        }
    }
}
