...
public class Door_Metal_1F_2_AnimationController : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            && ActionController.hitInfo.transform.name == gameObject.name)
        {
            if (Input.GetKeyDown(KeyCode.F))
            {
                ...
                photonView.RPC("RPC_ChangeMetalDoor_1F_2_OpenState", RpcTarget.All, viewId);
            }
        }
    }
    ...
    [PunRPC]
    void RPC_ChangeMetalDoor_1F_2_OpenState(int viewId)
    {
        PhotonView itemPhotonView = PhotonView.Find(viewId);

        if (itemPhotonView != null)
        {
            itemPhotonView.gameObject.GetComponent<Door_Metal_1F_2_AnimationController>().isDoorOpen =
                !itemPhotonView.gameObject.GetComponent<Door_Metal_1F_2_AnimationController>().isDoorOpen;
        }
    }
}
